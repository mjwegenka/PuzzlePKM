...
function CreatePanel({ createType, createKey, onTypeChange, onSave, onClose, onDirty, onNavigateToObject }: CreatePanelProps) {
  const blankObject =
...
export default function NotesPage({ onSaved }: NotesPageProps) {
  ...
  const filteredDailies = dailyNotes.filter(
    (n) => n.date.includes(dailyFilter) || n.preview?.toLowerCase().includes(dailyFilter.toLowerCase()),
  )
       ...
       <Stack direction="row" spacing={1.5} sx={{ flex: 1, minHeight: 0 }}>
         ...
         <NoteColumn
           ...
           items={filteredTopics.map((n) => ({
             ...
             secondary: n.updatedAt ? n.updatedAt.slice(0, 10) : undefined,
             tags: n.tags,
           }))}
           loading={loading}
           filter={topicFilter}
