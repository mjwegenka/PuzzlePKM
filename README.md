...
- **Library multi-select** – Gallery and list views support Shift+click range selection, Cmd/Ctrl+A select-all, bulk tag editing, and bulk delete for daily notes, topic notes, and habits (see [`DEC-70`](./IMPLEMENTATION_DECISIONS.md))
...
- **Sidebar Navigation** – Primary destinations are Library, Calendar, and Graph. Settings opens from the native macOS **PuzzlePKM** app menu as a dedicated window. Scripture discovery/listing is in existing Library/object-detail surfaces rather than a standalone page (see [`DEC-55`](./IMPLEMENTATION_DECISIONS.md) and [`DEC-62`](./IMPLEMENTATION_DECISIONS.md))
